...
fetch("tank_db.txt")
  ...
  .then(text => {
      ...
      TANKS_LOADED = true;
      console.log("Словарь танков загружен");
  });

// --- Ждём загрузки словаря перед обработкой ---
function waitTanksLoaded(callback){
    if(TANKS_LOADED){
        callback();
    } else {
        setTimeout(()=>waitTanksLoaded(callback), 50);
    }
}

// --- Нормализация имени танка ---
...
document.body.addEventListener("drop", e => {
    e.preventDefault();
    const files = Array.from(e.dataTransfer.files).filter(f=>f.name.endsWith(".mtreplay"));
    if(files.length>0) handleFiles(files);
});

// --- Input type="file" ---
document.getElementById("replayInput").addEventListener("change", e=>{
    const files = Array.from(e.target.files).filter(f=>f.name.endsWith(".mtreplay"));
    if(files.length>0) handleFiles(files);
});
